...
#include "crypto/block/mc-config-log-wrapper.h"
#include "git.h"
...
td::Result<block::Config> decode_config(const char* config_boc, int verbosity_level = 1) {
  // Set up log context to suppress debug messages during config unpacking when verbosity is 0
  std::unique_ptr<emulator::EmulatorLogContext> temp_log_context;
  if (verbosity_level == 0) {
    temp_log_context = std::make_unique<emulator::EmulatorLogContext>(verbosity_level);
  }
...
void *transaction_emulator_create(const char *config_params_boc, int vm_log_verbosity) {
  auto global_config_res = decode_config(config_params_boc, vm_log_verbosity);
...
void *emulator_config_create(const char *config_params_boc) {
  auto config = decode_config(config_params_boc, 0); // Use verbosity 0 for config creation
  if (config.is_error()) {
...
bool transaction_emulator_set_config(void *transaction_emulator, const char* config_boc) {
  ...
  auto global_config_res = decode_config(config_boc, 0); // Use verbosity 0 for config operations
  if (global_config_res.is_error()) {
    LOG(ERROR) << global_config_res.move_as_error().message();
    return false;
  }
...
void *tvm_emulator_create(const char *code, const char *data, int vm_log_verbosity) {
  // Set up log context early to suppress debug messages during emulator creation
  std::unique_ptr<emulator::EmulatorLogContext> temp_log_context;
  if (vm_log_verbosity == 0) {
    temp_log_context = std::make_unique<emulator::EmulatorLogContext>(vm_log_verbosity);
  }
